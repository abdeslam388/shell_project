#ifndef _SHELL_H_
#define _SHELL_H_

#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
#include <sys/types.h>
#include <sys/wait.h>
#include <sys/stat.h>
#include <limits.h>
#include <fcntl.h>
#include <errno.h>

/* for read/buffer writer */
#define READ_BUF_SIZE 1024
#define WRITE_BUF_SIZE 1024
#define BUF_FLUSH -1

/* for chaining */
#define CMD_NORM	0
#define CMD_OR		1
#define CMD_AND		2
#define CMD_CHAIN	3

/* convert_number()*/
#define CONVERT_LOWERCASE	1
#define CONVERT_UNSIGNED	2

/* 1 while using the system getline() */
#define USE_GETLINE 0
#define USE_STRTOK 0

#define HIST_FILE	".simple_shell_history"
#define HIST_MAX	4096

extern char **environ;


/**
 * struct liststr - "singly linked list"
 * @str: string "phrase"
 * @next: pointeur de node prochaine
 * @num: number nedded to be field
 */
typedef struct liststr
{
    int num;
    char *str;
    struct liststr *next;
}
 list_t;

/**
 * struct passinfo - contains pseudo-arguements to pass into a function,
 * allowing uniform prototype for function pointer struct
 * @arg: a string generated from getline contenant des arguments
 * @argv:un tableux de strings 
 * @path: a string path for the current command.
 * @argc: the argument counter
 * @line_count: the error 
 * @err_num: le code error  pour exit()s
 * @linecount_flag: if on counter this line of input
 * @fname: the program file-name
 * @env: linkedlist local copy of environ
 * @environ: custom modified copy of environ from LL environ
 * @history: the history 
 * @alias: the alias 
 * @env_changed: on if environ is in change
 * @status: the return status of the last exec'd command
 * @cmd_buf: address of pointer to cmd_buf, on if chaining
 * @cmd_buf_type: CMD_type ||, &&, ;
 * @readfd: the fd 
 * @histcount: the history numbers file reader
 */
typedef struct passinfo
{
    int argc;
    char **argv;
    char *arg;
    char *path;
    unsigned int line_count;
    int err_num;
    int linecount_flag;
    char *fname;
    list_t *history;
    list_t *alias;
    list_t *env;
    int env_changed;
    char **environ;
    int status;

    char **cmd_buf;
    int cmd_buf_type;
    int readfd;
    int histcount;
} info_t;
info_t;
#define INFO_INIT \
{NULL, NULL, NULL, 0, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, 0, 0, NULL, \
		0, 0, 0}

/**
 * struct builtin - contains a builtin string and related function
 * @type: the builtin command flag
 * @func: the function
 */
typedef struct builtin
{
	char *type;
	int (*func)(info_t *);
} builtin_table;


/* toem_shloop.c */
int hsh(info_t *, char **);
int find_builtin(info_t *);
void find_cmd(info_t *);
void fork_cmd(info_t *);

/* loophsh.c */
int loophsh(char **);

/*toem_path.c */
int is_cmd(info_t *, char *);
char *_dupchars(char *, int, int);
char *find_path(info_t *, char *, char *);

/* error_string_functions.c */
void _eputs(char *);
int _eputchar(char);
int _putfd(char c, int fd);
int _putsfd(char *str, int fd);

/*string_functionO.c */
int _strlen(char *);
int _strcmp(char *, char *);
char *starts_with(const char *, const char *);
char *_strcat(char *, char *);

/*string_function1.c */
char *_strcpy(char *, char *);
char *_strdup(const char *);
int _putchar(char);.
void _puts(char *);

/*string_function_ext.c*/
char *_strncpy(char *, char *, int);
char *_strncat(char *, char *, int);
char *_strchr(char *, char);

/*toem_str_tokenizer.c */
char **strtow(char *, char *);
char **strtow1(char *, char);

/* toem_realloc.c */
char *_memset(char *, char, unsigned int);
void ffree(char **);
void *_realloc(void *, unsigned int, unsigned int);

/*toem_memo.c */
int bfree(void **);

/*toem_functionsO.c*/
int interactive(info_t *);
int is_delim(char, char *);
int _isalpha(int);
int _isdigit(char *);

/*toem_functions1.c*/
int _errstin(char *);
void print_error(info_t *, char *);
int print_di(int, int);
char *convert_number(long int, int, int);
void rm_comments(char *);

/*toem_builtinO.c */
int _myexit(info_t *);
int _mycd(info_t *);
int _myhelp(info_t *);

/*toem_builtin1.c */
int _myhistory(info_t *);
int _myalias(info_t *);

/*toem_get_line.c  */
ssize_t get_input(info_t *);
int _getline(info_t *, char **, size_t *);
void sigintHandler(int);

/*toem_get_info.c */
void clear_info(info_t *);
void set_info(info_t *, char **);
void free_info(info_t *, int);

/*toem_environ.c */
char *get_env(info_t *, const char *);
int _myenv(info_t *);
int _mysetenv(info_t *);
int _myunsetenv(info_t *);
int populate_env_list(info_t *);

/*toem_get_env.c */
char **get_environ(info_t *);
int _myunsetenv(info_t *, char *);
int _mysetenv(info_t *, char *, char *);

/*toem_history.c */
char *get_history_file(info_t *info);
int write_history(info_t *info);
int read_history(info_t *info);
int build_history_list(info_t *info, char *buf, int linecount);
int renumber_history(info_t *info);

/*toem_listsO.c */
list_t *add_node(list_t **, const char *, int);
list_t *add_node_end(list_t **, const char *, int);
size_t print_list_str(const list_t *);
int delete_node_at_index(list_t **, unsigned int);
void free_list(list_t **);

/*toem_lists1.c */
size_t list_len(const list_t *);
char **list_to_strings(list_t *);
size_t print_list(const list_t *);
list_t *node_starts_with(list_t *, char *, char);
ssize_t get_node_index(list_t *, list_t *);

/* toem_vars.c*/
int is_chain(info_t *, char *, size_t *);
void check_chain(info_t *, char *, size_t *, size_t, size_t);
int replace_alias(info_t *);
int replace_virs(info_t *);
int replace_string(char **, char *);

#endif

